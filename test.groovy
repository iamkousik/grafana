    ...
    stages {
      ...
      stage('Docker Cluster Deployment') {
          steps {
            ...
            dir ("${WORKSPACE}") {
            sshCommand remote: remote, command: "mkdir -p /tmp/$__hostname__"
            sshPut remote: remote, from: ".\\docker\\${params.ModuleName}\\${DEPLOY_RELEASE}", into: "/tmp/$__hostname__/"
            sshCommand remote: remote, command: "ls -ltrh /tmp/$__hostname__/${DEPLOY_RELEASE}"
            sshCommand remote: remote, command: "cd /tmp/$__hostname__/${DEPLOY_RELEASE}; dos2unix Docker_Deployment.sh; sh Docker_Deployment.sh $__RepositoryName__:$__imageversion__ $__hostname__"
            sshRemove remote: remote, path: "/tmp/$__hostname__"
              }
            }
          }
        }
